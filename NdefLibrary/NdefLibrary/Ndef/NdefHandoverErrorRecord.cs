...
namespace NdefLibrary.Ndef
{
	...
	public class NdefHandoverErrorRecord : NdefRecord
	{
		...
		/// <returns>
		///     True if the record has the correct type, type name format and payload
		///     to be a Handover Error record, false if it's a different record.
		/// </returns>
		...
		///     Thrown if attempting to create a Handover Error record
		///     based on an incompatible record type.
		/// </exception>
		...
		///     The reason why the handover request could not be
		///     processed by the target device. The value of this defines the possible
		///     contents and content length of the second (errorData) parameter.
		/// </param>
		/// <param name="errorData">
		///     Additional data for the error reason. The contents
		///     depend on the error reason, and is usually a time value.
		/// </param>
