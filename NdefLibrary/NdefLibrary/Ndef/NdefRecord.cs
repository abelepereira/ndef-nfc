...
namespace NdefLibrary.Ndef
{
	...
	public class NdefRecord
	{
		...
		/// </summary>
		/// <remarks>
		///     The Type Name Format corresponds to the 3 least significant bits of the flags and can be calculated as:
		///     <c>
		...
		public enum TypeNameFormatType
		{
			...
			/// <summary>
			///     The media type indicates that the TYPE field contains a value that follows the media-type BNF construct defined by
			///     RFC 2046.
			/// </summary>
			Mime = 0x02,

			/// <summary>
			///     Absolute-URI indicates that the TYPE field contains a value that follows the absolute-URI BNF construct defined by
			///     RFC 3986.
			/// </summary>
			Uri = 0x03,

			/// <summary>
			///     NFC Forum external type indicates that the TYPE field contains a value that follows the type name format defined in
			///     [NFC RTD] for external type names.
			/// </summary>
			ExternalRtd = 0x04,

			/// <summary>
			///     Unknown SHOULD be used to indicate that the type of the payload is unknown. This is similar to the
			///     "application/octet-stream" media type defined by MIME.
		...
		/// </summary>
		/// <param name="tnf">
		///     Type name format to use, based on the tnf's standardized
		///     by the Nfc Forum.
		/// </param>
		...
		/// </summary>
		/// <param name="checkForSubtypes">
		///     If set to true, also checks for
		///     subtypes of the URL / SmartPoster record where the library offers
		///     a convenient handling class - e.g. for SMS or Mailto records,
		///     which are actually URL schemes.
		/// </param>
		/// <returns>
		///     Type name of the specialized class that can understand
		///     and manipulate the payload through convenience methods.
		/// </returns>
		...
		///     Thrown if no valid NDEF record can be
		///     created based on the record's current contents. The exception message
		///     contains further details about the issue.
		/// </exception>
		/// <returns>
		///     True if the record contents are valid, or throws an exception
		...
		public virtual bool CheckIfValid()
		{
			    ...
			    TypeNameFormat == TypeNameFormatType.Unknown)
			{
				...
				if (TypeNameFormat == TypeNameFormatType.Empty)
				{
